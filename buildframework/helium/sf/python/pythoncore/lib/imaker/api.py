...
_logger = logging.getLogger("imaker.api")
...
class IMakerConfigScanner(fileutils.AbstractScanner):
    ...
    def scan(self):
        ...
        p_open = subprocess.Popen("imaker help-config", shell=True, stdout=subprocess.PIPE)
        handle = p_open.communicate()[0]
...
def targets_for_config(config):
    ...
    p_open = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
    handle = p_open.communicate()[0]
    result = []
    for line in handle.splitlines():
        line = line.strip()
...
def get_variable(variable, target=None, product=None, config=None, default=None):
    ...
    _logger.info("imaker %s print-%s WORKDIR=%s" % (cmdline, variable, logdir))
    p_handle = subprocess.Popen("imaker %s print-%s WORKDIR=%s" % (cmdline, variable, logdir), shell=True, stdout=subprocess.PIPE)
    handle = p_handle.communicate()[0]
    result = []
    for line in handle.splitlines():
        line = line.strip()
        if line == "" or line.startswith("Total duration"):
