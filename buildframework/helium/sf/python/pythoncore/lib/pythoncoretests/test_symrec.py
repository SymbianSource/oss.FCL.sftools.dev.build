...
_logger = logging.getLogger("test.symrec")
logging.basicConfig()
...
def test_symrec_to_tdd():
    """test symrec to TDD"""
    filename = os.path.join(os.environ['TEST_DATA'], "data/symrec/GRACE/service/product/release_1_001", "release_metadata.xml")        
    _logger.info(symrec.Metadata2TDD(filename).to_tdd())
...
def test_ignore_whitespace_writexml():
    """ test ignore white space and write to XML"""
    myText = '''<foo>    
    ...
    print _logger.debug(result1)
    print _logger.debug(result2)
